...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WMP);
    ...
    if (!settings.contains("nAnonymizeWMPAmount"))
        settings.setValue("nAnonymizeWMPAmount", 1000);

    nAnonymizeWMPAmount = settings.value("nAnonymizeWMPAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeWMPAmount"))
        SoftSetArg("-anonymizewmpamount", settings.value("nAnonymizeWMPAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in wmp.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWMPAmount:
            return QVariant(nAnonymizeWMPAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWMPAmount:
            nAnonymizeWMPAmount = value.toInt();
            settings.setValue("nAnonymizeWMPAmount", nAnonymizeWMPAmount);
            emit anonymizeWMPAmountChanged(nAnonymizeWMPAmount);
            break;
        case CoinControlFeatures:
