...
// Copyright (c) 2017-2018 The WMP developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zwmpcontroldialog.h"
#include "ui_zwmpcontroldialog.h"
...
std::list<std::string> ZWMPControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZWMPControlDialog::listMints;

ZWMPControlDialog::ZWMPControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZWMPControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZWMPControlDialog::~ZWMPControlDialog()
{
    delete ui;
}

void ZWMPControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWMPControlDialog::updateList()
{
...
void ZWMPControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWMPControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZWMP_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWMPControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZWMPControlDialog::GetSelectedMints()
{
...
void ZWMPControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
